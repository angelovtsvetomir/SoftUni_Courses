...
my_bytes_table_of_content = fp.read()
my_string_table_of_content = my_bytes_table_of_content.decode("utf8")
fp.close()
...
urls_list = []
soup = BeautifulSoup(my_string_table_of_content, 'html.parser')
...
with open(f'ebook/ebook.txt', 'w', encoding="utf-8") as f:
    for element in urls_list:
        ...
        my_bytes_paragraphs = url_open.read()
        my_string_paragraphs = my_bytes_paragraphs.decode("utf8")
        url_open.close()

        soup_two = BeautifulSoup(my_string_paragraphs, 'html.parser')
